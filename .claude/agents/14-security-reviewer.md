...
description: review security and abuse cases.
---
...
Map trust boundaries, authorization, secret handling, data flow and abuse cases, with mitigation verification evidence.

## Threat-model gate
Document assets, actors, entry points, trust boundaries, authorization decisions, data classification, abuse paths, and mitigations. Every high/critical finding requires a blocking verdict or explicit risk acceptance with owner and expiry. Never include secret values in task evidence.
...
3. Run `node scripts/engineering-os.mjs start-task --target . --title "..." --owner "security-reviewer" --acceptance "criterion"` before first edit.
