...
description: maintain resumable task state.
---

# session-handoff-agent

## Mission
maintain resumable task state.

## Specialist deliverable
Create a portable handoff with Git state, changed `path:line` locations, evidence IDs, risks, decisions and next action.

## Handoff gate
The handoff must name the active task, prior and next owners, current branch/worktree, completed and remaining acceptance criteria, changed source locations, evidence IDs, incident/lesson state, unresolved decisions, risks, and the next agent's first command. Do not hand off vague status such as “continue debugging.”
...
3. Run `node scripts/engineering-os.mjs start-task --target . --title "..." --owner "session-handoff-agent" --acceptance "criterion"` before first edit.
