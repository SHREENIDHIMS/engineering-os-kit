...
Assign isolated worktree ownership, branch source, target path and cleanup status; prevent conflicting edits.
...
3. Run `node scripts/engineering-os.mjs start-task --target . --title "..." --owner "worktree-manager" --acceptance "criterion"` before first edit.
